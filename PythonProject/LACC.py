...
def LACC(input_img: np.ndarray, is_vid=False, is_run=False):
    ...
    # Получаем средние значения яркости для каждого канала
    means = cv2.mean(input_img)
    # Извлекаем средние значения яркости для каналов B, G и R
    b_mean, g_mean, r_mean = means[:3]
    # Разделяем изображение на каналы B, G и R
    b_channel, g_channel, r_channel = cv2.split(input_img)

    # Создаем список кортежей, содержащий среднее значение, канал и название канала
    channels = [[b_mean, b_channel, 'b'], [g_mean, g_channel, 'g'], [r_mean, r_channel, 'r']]

    # Сортируем каналы по среднему значению яркости
    small, medium, large = sorted(channels, key=lambda x: x[0])
